...
import ml.ozzo.Dao.CategoryRepository;
import ml.ozzo.Dao.ProductRepository;
import ml.ozzo.entities.Category;
   ...
   CommandLineRunner start(CategoryRepository categoryRepository, ProductRepository productRepository){
        return  args -> {
            ...
            Stream.of("P1","P2","P3","P4").forEach(name->{
              Product p=  productRepository.save(new Product(name,Math.random()*10000,c1));
              c1.getProducts().add(p);
              categoryRepository.save(c1);
            });
            Category c2=categoryRepository.findById("C2").get();
            Stream.of("P5","P6","P7","P8").forEach(name->{
                Product p =productRepository.save(new Product(name,Math.random()*10000,c2));
